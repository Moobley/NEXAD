...
import { DashboardUi } from "@/components/sections/work/ui-fragments"

export async function BarberBusiness() {
  const t = await getTranslations("projects.barber.lab.business")
  const ts = await getTranslations("projects.barber.lab.sides")
  const features = t.raw("features") as string[]
  const business = ts.raw("business") as { flow: string[] }
      ...
      <div className="mx-auto w-full max-w-[1600px] px-6 py-24 md:px-10 md:py-32">
        <div className="grid gap-14 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-7">
            <Reveal delay={120}>
              <CaseMedia label={t("eyebrow")} tone="dark" className="aspect-[4/3]">
                <DashboardUi blocks={business.flow} />
              </CaseMedia>
            </Reveal>
          </div>
        </div>
